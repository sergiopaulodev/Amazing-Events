...
// Así convierto la fecha en entero para poder compararla
let x = data.currentDate.replaceAll('-','');
let numberCurrentDate = parseInt(x);
// console.log(typeof(numberCurrentDate));

let upComing = []
// let PastEvent = []

for (let i = 0; i <=13; i++){
  let numberDate = data.events[i].date.replaceAll('-','');
  if(numberDate >= numberCurrentDate){
...
// for of: lograr que en cada iteracion me imprima cada card


for (let i = 0; i <= 0; i++){
  
    let imgCard = document.getElementById('img-0')
    imgCard.src = upComing[i].image
    
    let titleCard = document.getElementById('title-card-0');
    titleCard.innerHTML = upComing[i].name
    
    let descriptionCard = document.getElementById('p-card-0')
    descriptionCard.innerHTML = upComing[i].description
    
    let price = document.getElementById('price-0')
    price.innerHTML = `Price: $${upComing[i].price}`;
